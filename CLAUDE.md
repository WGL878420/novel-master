...
### 写前准备
1. `python3 scripts/story_graph.py -p <项目> brief --chapter N` — 最新状态简报
2. `python3 scripts/story_graph.py -p <项目> check-continuity` — 检查属性矛盾和伏笔超期
3. `bible/03-outline.md` — 确认当前章节在主线中的位置
4. 如果章纲只剩 ≤3 章 → 先 `extend-outline --count 20` 再写
...
### 写后同步（防跑偏的关键）
7. `python3 scripts/story_graph.py -p <项目> post-write --chapter N` —
   - 自动更新时间线（timeline.md）
   - 自动同步事件矩阵
